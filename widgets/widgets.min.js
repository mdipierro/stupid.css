...
format_time=format_time.split(' ')[1];var timeout=null;var set=function(){s.val(t.val().trim().length&&date&&date.format(ISO)||'').change();};var update=(function(){date=Date.create(t.val());redraw(true);}).debounce(100);var func=function(dt){return function(e){e.stopPropagation();date=Date.create(q);if(dt['minutes'])
...
w.html(html).find('td').css({'text-align':'center'});w.find('.cal-ym').click(func({years:-1}));w.find('.cal-yp').click(func({years:+1}));w.find('.cal-mm').click(func({months:-1}));w.find('.cal-mp').click(func({months:+1}));w.find('.cal-Hm').click(func({hours:-1}));w.find('.cal-Hp').click(func({hours:+1}));w.find('.cal-mim').click(func({minutes:-5}));w.find('.cal-mip').click(func({minutes:+5}));w.find('.cal-day').click(function(e){e.stopPropagation();date=q;var d=parseInt(jQuery(this).text());date.set({day:d});redraw();});w.find('.cal-y,.cal-t,.cal-selected').css('font-weight','bold');jQuery('.cal-wrapper').not(w).hide();w.fadeIn();}
var close=function(e){var q=jQuery(e.target);if(q.is('input')||q.closest('.cal-wrapper').length>0)return;set();if(timeout)clearTimeout(timeout);timeout=setTimeout(function(){if(w.find('td:hover').length==0)w.fadeOut();},300);};t.removeAttr('name').after(s.prop('name',t.prop('name')));t.keyup(update).blur(close).click(function(e){e.stopPropagation();redraw();});jQuery('html').on('click',close);w.click(function(e){e.stopPropagation();});w.css({'position':'absolute','z-index':3000,'cursor':'pointer','top':(t.offset().top+t.height()+10)+'px','left':(t.offset().left)+'px','background':'white','box-shadow':'0 0 12px #ddd'});t.val(date.format(format)).after(w);set();};jQuery(function(){jQuery('.date,.datetime,.time').calendar();});
...
onchange();t.keyup(onchange);t.dblclick(update);t.after(s);var isDragging=false;t.mousedown(function(){isDragging=true;});t.mousemove(function(e){if(isDragging)update(e);});t.mouseup(function(){isDragging=false;});jQuery(window).resize(function(){s.css({top:t.offset().top+'px',left:t.offset().left+'px'});});};jQuery(function(){jQuery('.slider').slider();});
jQuery('input.stars').each(function(){var span=jQuery('<span class="stars-wrapper" style="white-space:nowrap"><span data-value="1">&#x2606;</span><span data-value="2">&#x2606;</span><span data-value="3">&#x2606;</span><span data-value="4">&#x2606;</span><span data-value="5">&#x2606;</span></span>');var self=jQuery(this).hide().after(span);span.css('padding-left',self.css('padding-left'));var fill_stars=function(){var k=parseInt(self.val())||0;for(var i=1;i<6;i++)
span.find('[data-value='+i+']').html((i<=k)?'&#x2605;':'&#x2606;');};for(var k=0;k<6;k++)(function(k){span.find('[data-value='+k+']').mouseover(function(){for(var i=1;i<6;i++)
span.find('[data-value='+i+']').html((i<=k)?'&#x2605;':'&#x2606;');}).click(function(){self.val(k).change();fill_stars();});})(k);span.mouseout(fill_stars);fill_stars();});
$.fn.tags=function(){var t=this;var sep=t.attr('data-separator')||',';;var tags=t.val().split(sep);var options=t.attr('data-options');var w=jQuery('<div class="tags dismissible"/>');var s=jQuery('<input/>').hide();options=options&&options.split(sep)||[];w.css('padding-left',t.css('padding-left'));s.attr('name',t.attr('name'));t.val('').before(w).after(s);t.removeAttr('name');var tag=function(name){return jQuery('<span>'+name+'</span>').attr('data-tag-name',name);};var show=function(){w.html('');for(var i=0;i<tags.length;i++)
w.append(tag(tags[i])).append(' ');for(var i=0;i<options.length;i++)
if(tags.indexOf(options[i])<0)
w.append(tag(options[i]).addClass('off')).append(' ');s.val(tags.join(sep)).change();};t.keyup(function(e){e.preventDefault();var code=e.keyCode||e.which;if(code==13||code==188){var k=t.val().trim().replace(/(^[,;:-]+|[,;:-]+$)/,'');if(k&&tags.indexOf(k)<0){w.append(tag(k)).append(' ');tags.push(k);show();}
t.val('');};});w.on('click','span:not(.off)',function(){var p=jQuery(this);var name=p.attr('data-tag-name');tags.splice(tags.indexOf(name),1);if(options.indexOf(name)<0)options.unshift(name);show();p.remove();});w.on('click','span.off',function(){var p=jQuery(this);tags.push(p.attr('data-tag-name'));show();p.remove();});show();};jQuery(function(){jQuery('input.tags').tags();});
