...
var close=function(e){if(jQuery(e.target).is('input'))return;set();if(timeout)clearTimeout(timeout);timeout=setTimeout(function(){if(t.not(':focus')&&w.find('td:hover').length==0)w.fadeOut();},300);};t.removeAttr('name').after(s.prop('name',t.prop('name')));t.keyup(update).blur(close).click(redraw);jQuery('body').on('click',close);w.css({'position':'absolute','z-index':3000,'cursor':'pointer','top':(t.offset().top+t.height()+10)+'px','left':(t.offset().left)+'px','background':'white','box-shadow':'0 0 12px #ddd'});t.val(date.format(format)).after(w);set();};jQuery(function(){jQuery('.date,.datetime,.time').calendar();});
...
$.fn.tags=function(){var t=this;var sep=t.attr('data-separator')||'|';;var tags=t.val().split(sep);var w=jQuery('<div class="tags"/>');var s=jQuery('<input/>').hide();var tag=function(name){return jQuery('<span>'+name+'</span>').append('&nbsp;<i class="fa fa-remove"></i>').attr('data-tag-name',name);}
...
t.val('');};});w.on('click','span i',function(){var p=jQuery(this).parent();tags.splice(tags.indexOf(p.attr('data-tag-name')),1);s.val(tags.join(sep));p.remove();});};jQuery(function(){jQuery('input.tags').tags();});
