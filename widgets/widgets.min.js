...
return this.each(function(){$(this).calendar();});var ISO='{yyyy}-{MM}-{dd} {hh}:{mm}:{ss}';var t=this;var s=$('<input type="hidden"/>');var w=$('<table class="cal-wrapper"/>');var value=t.val();var date=Date.create(value);var start=t.data('start')||'Sunday';var format=t.data('format')||'{dd}/{MM}/{year} {hh}:{mm} {TT}';var format_time=format;if(format_time.indexOf('{year}')>=0)
...
w.html(html).find('td').css({'text-align':'center'});w.find('.cal-ym').click(func({years:-1}));w.find('.cal-yp').click(func({years:+1}));w.find('.cal-mm').click(func({months:-1}));w.find('.cal-mp').click(func({months:+1}));w.find('.cal-Hm').click(func({hours:-1}));w.find('.cal-Hp').click(func({hours:+1}));w.find('.cal-mim').click(func({minutes:-5}));w.find('.cal-mip').click(func({minutes:+5}));w.find('.cal-day').click(function(e){e.stopPropagation();date=q;var d=parseInt($(this).text());date.set({day:d});redraw();});w.find('.cal-y,.cal-t,.cal-selected').css('font-weight','bold');$('.cal-wrapper').not(w).hide();w.fadeIn();}
var close=function(e){var q=$(e.target);if(q.is('input')||q.closest('.cal-wrapper').length>0)return;set();if(timeout)clearTimeout(timeout);timeout=setTimeout(function(){if(w.find('td:hover').length==0)w.fadeOut();},300);};t.removeAttr('name').after(s.prop('name',t.prop('name')));t.keyup(update).blur(close).click(function(e){e.stopPropagation();redraw();});$('html').on('click',close);w.click(function(e){e.stopPropagation();});w.css({'position':'absolute','z-index':3000,'cursor':'pointer','top':(t.offset().top+t.height()+10)+'px','left':(t.offset().left)+'px','background':'white','box-shadow':'0 0 12px #ddd'});t.val(date.format(format)).after(w);set();};$(function(){$('.date,.datetime,.time').calendar();});
...
$.fn.tags=function(){var t=this;if(this.length>1)
return this.each(function(){$(this).tags();});var sep=t.attr('data-separator')||',';;var tags=t.val().split(sep);var options=t.attr('data-options');var w=$('<div class="tags dismissible"/>');var s=$('<input/>').hide();options=options&&options.split(sep)||[];w.css('padding-left',t.css('padding-left'));s.attr('name',t.attr('name'));t.val('').before(w).after(s);t.removeAttr('name');var tag=function(name){return $('<span>'+name+'</span>').attr('data-tag-name',name);};var show=function(){w.html('');for(var i=0;i<tags.length;i++)
...
t.val('');};});w.on('click','span:not(.off)',function(){var p=$(this);var name=p.attr('data-tag-name');tags.splice(tags.indexOf(name),1);if(options.indexOf(name)<0)options.unshift(name);show();p.remove();});w.on('click','span.off',function(){var p=$(this);tags.push(p.attr('data-tag-name'));show();p.remove();});show();};$(function(){$('input.tags').tags();});
