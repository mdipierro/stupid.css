...
var close=function(){set();if(timeout)clearTimeout(timeout);timeout=setTimeout(function(){console.log('checking:'+w.is(':hover'));if(w.find('td:hover').length==0)w.fadeOut();},300);};t.removeAttr('name').after(s.prop('name',t.prop('name')));t.keyup(update).blur(close).click(redraw);jQuery('body').on('click',close);w.css({'position':'absolute','z-index':3000,'cursor':'pointer','top':(t.offset().top+t.height()+10)+'px','left':(t.offset().left)+'px','background':'white','box-shadow':'0 0 12px #ddd'});t.val(date.format(format)).after(w);set();};jQuery(function(){jQuery('.date,.datetime,.time').calendar();});
