...
format_time=format_time.split(' ')[1];var timeout=null;var set=function(){s.val(t.val().trim().length&&date&&date.format(ISO)||'').change();};var update=(function(){date=Date.create(t.val());redraw(true);}).debounce(100);var func=function(dt){return function(e){e.stopPropagation();date=Date.create(q).advance(dt);redraw()};};var tr=function(x){return'<tr>'+x+'</tr>';}
...
jQuery('input.stars').each(function(){var span=jQuery('<span style="white-space:nowrap"><span class="rate0">&#x25CE;</span><span class="rate1">&#x2606;</span><span class="rate2">&#x2606;</span><span class="rate3">&#x2606;</span><span class="rate4">&#x2606;</span><span class="rate5">&#x2606;</span></span>');var self=jQuery(this).hide().after(span);var fill_stars=function(){var k=parseInt(self.val())||0;for(var i=1;i<6;i++)span.find('.rate'+i).html((i<=k)?'&#x2605;':'&#x2606;');};for(var k=0;k<6;k++)(function(k){span.find('.rate'+k).mouseover(function(){for(var i=1;i<6;i++)span.find('.rate'+i).html((i<=k)?'&#x2605;':'&#x2606;');}).click(function(){self.val(k).change();fill_stars();});})(k);span.mouseout(fill_stars);fill_stars();});
$.fn.tags=function(){var t=this;var sep=t.attr('data-separator')||'|';;var tags=t.val().split(sep);var w=jQuery('<div class="tags"/>');var s=jQuery('<input/>').hide();var tag=function(name){return jQuery('<span>'+name+'</span>').append('&nbsp;<b>✕</b>').attr('data-tag-name',name);}
s.val(tags.join(sep)).change();s.attr('name',t.attr('name'));t.val('').before(w).after(s);t.removeAttr('name');for(var i=0;i<tags.length;i++)w.append(tag(tags[i]));t.keyup(function(e){e.preventDefault();var code=e.keyCode||e.which;if(code==32||code==13||code==188||code==186||code==9){var k=t.val().trim();if(k){w.append(tag(k));tags.push(k);s.val(tags.join(sep)).change();}
t.val('');};});w.on('click','span b',function(){var p=jQuery(this).parent();tags.splice(tags.indexOf(p.attr('data-tag-name')),1);s.val(tags.join(sep)).change();p.remove();});};jQuery(function(){jQuery('input.tags').tags();});
