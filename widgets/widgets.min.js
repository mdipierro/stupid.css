...
format_time=format_time.split(' ')[1];var timeout=null;var set=function(){s.val(t.val().trim().length&&date&&date.format(ISO)||'');};var update=(function(){date=Date.create(t.val());redraw(true);}).debounce(100);var func=function(dt){return function(e){e.stopPropagation();date=Date.create(q).advance(dt);redraw()};};var tr=function(x){return'<tr>'+x+'</tr>';}
...
s.val(tags.join(sep));s.attr('name',t.attr('name'));t.val('').before(w).after(s);t.removeAttr('name');for(var i=0;i<tags.length;i++)w.append(tag(tags[i]));t.keyup(function(e){e.preventDefault();var code=e.keyCode||e.which;if(code==32||code==13||code==188||code==186||code==9){var k=t.val().trim();if(k){w.append(tag(k));tags.push(k);s.val(tags.join(sep));}
t.val('');};});w.on('click','span b',function(){var p=jQuery(this).parent();tags.splice(tags.indexOf(p.attr('data-tag-name')),1);s.val(tags.join(sep));p.remove();});};jQuery(function(){jQuery('input.tags').tags();});
