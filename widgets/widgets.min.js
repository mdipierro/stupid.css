...
s.val(tags.join(sep));t.val('').before(w).after(s);s.attr('name',t.attr('name'));t.removeAttr('name');for(var i=0;i<tags.length;i++)w.append(tag(tags[i]));t.keyup(function(e){e.preventDefault();var code=e.keyCode||e.which;if(code==32||code==13||code==188||code==186||code==9){var k=t.val().trim();if(k){w.append(tag(k));tags.push(k);s.val(tags.join(sep));}
