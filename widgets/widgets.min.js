...
return this.each(function(){$(this).calendar();});var swap=function(x){var w=$('<input/>').addClass(x.attr('class')).attr('style',x.attr('style'));x.hide().after(w);return w;};var ISO='{yyyy}-{MM}-{dd} {hh}:{mm}:{ss}';var self=this;var input=swap(self);var popup=$('<table class="cal-wrapper"/>');var value=self.val();var date=Date.create(value);var start=self.data('start')||'Sunday';var format=self.data('format')||'{dd}/{MM}/{year} {hh}:{mm} {TT}';var format_time=format;if(format_time.indexOf('{year}')>=0)
...
return this.each(function(){$(this).tags();});var swap=function(x){var w=$('<input/>').addClass(x.attr('class')).attr('style',x.attr('style'));x.hide().after(w);return w;};var self=this;var sep=self.data('separator')||',';;var tags=self.val().split(sep);var options=self.data('options');var container=$('<div class="tags dismissible"/>');var input=swap(this);options=options&&options.split(sep)||[];container.css('padding-left',self.css('padding-left'));self.after(container);var tag=function(name){return $('<span>'+name+'</span>').data('tag-name',name);};var show=function(){container.html('');for(var i=0;i<tags.length;i++)
...
input.val('');};});container.on('click','span:not(.off)',function(){var p=$(this);var name=p.data('tag-name');tags.splice(tags.indexOf(name),1);if(options.indexOf(name)<0)options.unshift(name);show();p.remove();});container.on('click','span.off',function(){var p=$(this);tags.push(p.data('tag-name'));show();p.remove();});show();};$(function(){$('input.tags').tags();});
