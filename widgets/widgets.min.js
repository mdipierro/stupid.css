...
$.fn.tags=function(){var t=this;var sep=t.attr('data-separator')||',';;var tags=t.val().split(sep);var options=t.attr('data-options');var w=jQuery('<div class="tags"/>');var s=jQuery('<input/>').hide();options=options&&options.split(sep)||[];w.css('padding-left',t.css('padding-left'));s.attr('name',t.attr('name'));t.val('').before(w).after(s);t.removeAttr('name');var tag=function(name){return jQuery('<span>'+name+'</span>').append('&nbsp;<b>✕</b>').attr('data-tag-name',name);};var off=function(name){return jQuery('<span class="off">'+name+'</span>').attr('data-tag-name',name);};var show=function(){w.html('');for(var i=0;i<tags.length;i++)
w.append(tag(tags[i])).append(' ');for(var i=0;i<options.length;i++)
if(tags.indexOf(options[i])<0)
w.append(off(options[i])).append(' ');s.val(tags.join(sep)).change();};t.keyup(function(e){e.preventDefault();var code=e.keyCode||e.which;if(code==13||code==188){var k=t.val().trim().replace(/(^[,;:-]+|[,;:-]+$)/,'');if(k){w.append(tag(k)).append(' ');tags.push(k);show();}
t.val('');};});w.on('click','span b',function(){var p=jQuery(this).parent();tags.splice(tags.indexOf(p.attr('data-tag-name')),1);show();p.remove();});w.on('click','span.off',function(){var p=jQuery(this);tags.push(p.attr('data-tag-name'));show();p.remove();});show();};jQuery(function(){jQuery('input.tags').tags();});
