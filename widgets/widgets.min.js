...
$.fn.tags=function(){var t=this;var sep=t.attr('data-separator')||'|';;var tags=t.val().split(sep);var w=jQuery('<div class="tags"/>');w.css('padding-left',t.css('padding-left'));var s=jQuery('<input/>').hide();var tag=function(name){return jQuery('<span>'+name+'</span>').append('&nbsp;<b>✕</b>').attr('data-tag-name',name);}
s.val(tags.join(sep)).change();s.attr('name',t.attr('name'));t.val('').before(w).after(s);t.removeAttr('name');for(var i=0;i<tags.length;i++)w.append(tag(tags[i])).append(' ');t.keyup(function(e){e.preventDefault();var code=e.keyCode||e.which;console.log(code);if(code==13||code==188){var k=t.val().trim().replace(/(^[,;:-]+|[,;:-]+$)/,'');if(k){w.append(tag(k)).append(' ');tags.push(k);s.val(tags.join(sep)).change();}
